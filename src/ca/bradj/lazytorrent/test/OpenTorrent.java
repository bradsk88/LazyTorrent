...
//	private static final String OPEN_TORRENT = TORRENT_PROG + " /MINIMIZED /DIRECTORY \"D:" + File.separator + "\" \"";
	...
	public static Result ofFile(File f, String torrentCommand) {
		if (f.exists()) {
			try {
				String command = torrentCommand + " " + f.getAbsolutePath();
				if (torrentCommand.contains("%TORRENT%")) {
					command = torrentCommand.replace("%TORRENT%", f.getAbsolutePath());
				}
